...
=======*/
		...
		if (t.getEndDate().equals(date)){
			s += "<p><font color=\"#FF0A0A\"><b>"
					+ Local.getString("Should be done today") //This is the autogenerated message 
					+ ".</b></font></p>";
		}
		else {
			Calendar endDateCal = t.getEndDate().getCalendar();
			...
			int numOfWeeks = ((endDateCal.get(Calendar.YEAR) + endDateCal.get(Calendar.DAY_OF_YEAR)) - 
					(dateCal.get(Calendar.YEAR) + dateCal.get(Calendar.DAY_OF_YEAR)))/7;
			String days = "";
			if(numOfDays > 0) {
				if(numOfDays >= 7){
					days = Local.getString("in")+" "+numOfWeeks+" "+Local.getString("week(s)");
					s += "<p><font color=\"#2A13F6\"><b>"
							+ days + " "
							+ ".</b></font></p>";
				} 
				else if (numOfDays >= 1) {
